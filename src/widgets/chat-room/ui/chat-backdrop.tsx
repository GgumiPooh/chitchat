...
export function ChatBackdrop({ className, mediaId, blurhash }: ChatBackdropProps) {
  return (
    ...
    <div className={cn("pointer-events-none absolute inset-x-0 top-0 h-[100lvh]", className)}>
      ...
      {/* WARN: `toChromeTint` carries this same 40%, because the colour bordering iOS 26's chrome is the photo under this wash and not the photo. Changing it here alone leaves the status bar a shade off the room it is meant to disappear into. */}
      <div className="absolute inset-0 bg-chat-scrim/40" />
    </div>
  );
}
