...
export function DayCell({ className, cell, isToday, isSelected, onSelect }: DayCellProps) {
  return (
    <span className={cn("group relative flex", className)}>
      ...
      {/* WARN: `keepsScroll` is what makes the overlay survivable on a tiled cell — without it the switch tracks the drag itself and the shell stops scrolling (`DESIGN.md § 7.15.`). */}
