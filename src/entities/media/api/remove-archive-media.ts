...
/**
 * REQUIREMENTS.md § 18. #1. What the library's 삭제 does.
 *
 * The chat is the record and the library is a view of it, so removing a photo
 * that a message carries is scoped to the library: `gallery_hidden_at` is set and
 * the bubble goes on rendering it. A photo no message carries — one uploaded
 * straight into the library (§ 10.) — has nothing left to render it, so the row
 * and its two R2 objects are deleted outright rather than left unreachable in the
 * bucket forever.
 *
 * WARN: Deliberately not scoped to the uploader. The library belongs to the
...
export async function removeArchiveMedia(ids: MediaId[]): Promise<ArchiveRemoval> {
  ...
  // WARN: This does *not* protect a library upload whose `postMessage` is still in flight — § 10. registers those with `gallery_added_at` set, so they are in the grid with no `message_media` child and read as orphans here. The screen is what closes that window: `useArchiveUpload` reports `isBusy` until the post settles, and selection is unavailable while it does.
