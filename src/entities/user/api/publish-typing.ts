...
/**
 * REQUIREMENTS.md § 8.12. Announces that this user is composing. Nothing is
 * written and nothing is read back — a client that hears it has everything the
 * indicator needs.
 *
 ...
 * (§ 8.12.).
 */
export async function publishTyping(userId: string): Promise<void> {
  await notifyChannel(TYPING_CHANNEL, JSON.stringify(typingEventSchema.parse({ userId })));
}
