...
import { DELETED_MESSAGE_TEXT } from "./app";
import { toLlmProviderBranding } from "./llm";
import { toMediaCountUnit, toMediaLabel, type MediaNoun } from "./media";
...
/** The fields of a `ReplyPreview` the summary line reads — structural, so this stays in `shared` and out of the entity's `server-only` barrel. */
export type QuoteSummarySource = {
  kind: "text" | "media" | "emoticon" | "system";
  isDeleted: boolean;
 ...
 */
export function toReplySummary(replyTo: QuoteSummarySource): string {
 ...
 */
function toMediaSummary({ mediaKind, mediaCount }: QuoteSummarySource): string {
