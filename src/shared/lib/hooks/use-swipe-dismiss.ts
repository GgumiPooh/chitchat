...
export type SwipeDismissOptions = {
  ...
  onDismiss: () => void;
  /**
   * A drag that engaged and then ended without dismissing — sprung back, or cancelled.
   *
   * WARN: DESIGN.md § 7.10. What the § 7.10. viewer does with it is repair its own scroll offset, and it is not optional there. The surface this rides is a native `snap-x` scroller the browser pans on its own threshold, so a drag it also claimed leaves `scrollLeft` somewhere nobody chose — the reader let go of a pull and found themselves tens of slides along the track. Fired only where a drag actually engaged, so a tap or a refused gesture asks the caller for nothing.
   */
  onSettle?: () => void;
};
 ...
 */
export function useSwipeDismiss({ isEnabled, canStart, onDismiss, onSettle }: SwipeDismissOptions) {
  ...
  function abandon(event: PointerEvent<HTMLElement>) {
    ...
    // WARN: Only where a drag had engaged. This function also answers a second finger and a mid-drag `isEnabled` flip, neither of which has moved anything for the caller to repair.
    if (isDragging) {
      onSettle?.();
    }
  }
