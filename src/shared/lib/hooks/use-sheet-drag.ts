...
export function useSheetDrag({
  ...
  // WARN: A single-frame flag that is true only in the render where the 300ms
  ...
  // WARN: The pending close is cancelled here rather than in the reset below, which reads and writes it during render (`react-hooks/refs`). `useLayoutEffect` and not `useEffect`: a passive effect is a task of its own, and the 300ms timer can fire in front of one and close a sheet that has just re-opened.
  useLayoutEffect(() => {
