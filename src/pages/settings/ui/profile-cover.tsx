...
export function ProfileCover({
  ...
  return (
    <div
      className={cn(
        // WARN: `shrink-0` — this is the only item in the settings column with real shrink headroom (480px against ~16px per row), so an overflowing screen would absorb all of it here and collapse the band DESIGN.md § 7.16. specifies. `RouteTransition`'s spacer carries the same warning for the same reason.
        "relative flex shrink-0 flex-col items-center justify-end gap-sm overflow-hidden pb-lg",
        // WARN: DESIGN.md § 7.16. A fixed height and not any viewport unit — the band is a background, so nothing the keyboard or the device size moves may reach it, and half a viewport was a different band on every phone.
        "h-120",
