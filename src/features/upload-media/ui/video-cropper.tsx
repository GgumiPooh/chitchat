...
import { MAX_BACKGROUND_VIDEO_SIZE } from "@/shared/config";
import { cn, formatSize, type Nullable } from "@/shared/lib";
...
/**
 * REQUIREMENTS.md § 12.1. Frames a video the way `MediaEditor` frames a photo, for
 * the one target a clip can be worn on — the profile cover.
 *
 ...
 *
 * INFO: Free-form, with no ratio chips: the only slot this reaches is a background,
 * which is drawn `object-cover` at two geometries (§ 12.2.), so a fixed ratio would
 * crop it twice.
 *
 * WARN: `absolute`, never `fixed` — `ShellOverlay` owns the viewport-sized box this
 * fills (DESIGN.md § 3.3.), exactly as `MediaEditor` does.
 */
export function VideoCropper({ className, draft, onCancel, onDone }: VideoCropperProps) {
