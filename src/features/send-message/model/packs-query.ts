import { fetchPacksWithItems } from "../api/fetch-packs";

/**
 * REQUIREMENTS.md § 13.6. The one description of the pack list, shared by the panel
 * that renders it, the composer that matches keywords against it, and the preload
 * that has to reach it first.
 *
...
export function toEmoticonPacksQuery() {
  return {
    queryKey: ["emoticon-packs", "with-items"] as const,
    queryFn: fetchPacksWithItems,
  };
}
