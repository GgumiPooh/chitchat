...
| Jump landing    | The tile a `?target=` jump landed on wears a 2px `primary` inset ring for `MESSAGE_FLASH_DURATION`, fading out over 300ms (§ 6.8.). A ring rather than that flash's `primary-tint` wash — a photograph fills the cell, so nothing behind it shows — and never a dim, per the row above |
