...
**That 45% is written in two places and it is one value.** The wash is `chat-scrim/45` over the photo; `useChromeTint` composites the same 45% of the same token over the wallpaper's average colour to produce the § 3.3. status-bar and toolbar tint. What borders iOS 26's chrome is the photo _under_ the wash rather than the photo, so the bare average publishes a colour visibly deeper than the room beside it — change one of the two alone and the bars sit a shade off the screen they are meant to disappear into. The tint takes that average from the stored blurhash and composites it in CSS (`REQUIREMENTS.md § 12.2.`), which is what keeps it following `chat-scrim` through the theme swap this paragraph's own token exists for.
