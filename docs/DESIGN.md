...
| Full-bleed photos   | `--layout-viewport-height`, top-anchored and clipped — a wallpaper must **not** follow the keyboard (§ 7.16.)                                                                               |
...
Why one thing in the shell is sized to the _layout_ viewport after all: everything above follows the keyboard because it is chrome or content, and both have to stay reachable. A full-bleed photo is neither. Sized to the shell it is `object-cover` over a box that loses a third of its height, so every frame of the keyboard sliding re-crops and rescales it — the wallpaper visibly zooms behind a conversation that is only making room for the keys. `--layout-viewport-height` is `documentElement.clientHeight`, published by the same sync, and a photo held at that height and anchored to the top is simply **clipped** by the shrinking box in front of it: the image itself does not move. The clipping ancestor is required, or a box standing taller than its parent extends `#app-scroll`'s scroll range into a phantom scroll.
...
| Stack clearance | **Every row that can top the stack clears the history by that same `xs`** — the reply bar (§ 6.10.), the media tray, the emoticon panel. They are alternatives for one position, so a gap only one of them carries reads as the other two being jammed into the bubble above. Each renders nothing when empty, so the resting composer is unchanged                      |
...
**The wallpaper is the one thing in the shell that does not follow the keyboard** (§ 3.4.). Sized to the room it is `object-cover` over a box that loses a third of its height the moment the composer takes focus, so the photo re-crops and rescales on every frame of the 200ms the keyboard slides — a wallpaper visibly zooming behind a conversation that is only making room for the keys. It is held at `--layout-viewport-height` and anchored to the top instead, so the shrinking room **clips** it and the image itself does not move. The wash stays on the room's own box rather than on the photo: it answers for the contrast of what is on screen, not for the strip the keyboard has covered. The clipping ancestor is not optional — a box standing taller than its parent would extend `#app-scroll`'s scroll range into a phantom scroll on the one screen that must not have one.
