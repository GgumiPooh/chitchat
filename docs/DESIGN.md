...
| Full-bleed photos   | `50lvh` where the box _is_ the background; `--layout-viewport-height` top-anchored and clipped where it sits behind a screen (§ 7.16.)                                                      |
...
Which unit, and it is not one answer. Where the box **is** the background it simply takes `lvh`: the large viewport is the only one no keyboard moves on either engine, since `dvh` and the layout viewport both shrink under Chromium's `interactive-widget=resizes-content` (which this app sets, § 3.4.). Where the photo sits **behind a screen** that must keep following the keyboard, the photo is held at `--layout-viewport-height` — `documentElement.clientHeight`, published by the same sync — and anchored to the top, so the shrinking box in front simply **clips** it and the image does not move. That case needs a clipping ancestor, or a box standing taller than its parent extends `#app-scroll`'s scroll range into a phantom scroll.
...
**The Settings header** is a cover band **half the large viewport tall** — `50lvh`, and no longer `--viewport-height`. The band is a background, so the keyboard must not resize it (§ 3.4.), and `lvh` is the only unit no keyboard moves on either engine: `--viewport-height` is the visual viewport and loses a third of its height to the § 12.1. editor sheet, while `dvh` and `--layout-viewport-height` both shrink under Chromium's `interactive-widget=resizes-content`, which this app sets.

`50vh` was originally rejected here on two grounds and **only one of them was true**. It does not shrink for the keyboard — recorded as a defect, and it is the behaviour actually wanted. It "ignores the browser chrome the shell is sized around" — that one held, and it is the reason the unit is `lvh` and not `vh`: with a URL bar showing, `vh` and `lvh` are the same too-tall value, so this band is a little over half of what is visible in a browser tab. It is exact in the installed PWA, which has no retractable chrome, and that is the platform § 16.1. targets. The photo is `object-cover`, the avatar and the name sit at the bottom of the band, and the band runs **full-bleed from the top of the screen**: it deliberately does not clear the floating 설정 header, because its own top scrim is what keeps that title legible over a photo.
...
**Where the box itself is the background, the box takes the stable height.** The Settings band is the whole cover, so it is `50lvh` outright and the photo inside it is a plain `inset-0` — no custom property, no clipping, nothing to keep in step. Stabilising only the photo and leaving the band on `--viewport-height` was tried and is not a fix: the picture stops rescaling and the band goes on collapsing around it, which to look at is the same thing.

**Where the box is a screen the photo merely sits behind** — the chat room, the profile screen — the box must keep following the keyboard, because its content has to stay reachable. There the photo alone is held at `--layout-viewport-height`, anchored to the top, and **clipped** by the shrinking box in front of it. `lvh` would serve there too and would additionally survive `interactive-widget=resizes-content`; the property is kept because those two want _the box's own resting height_ rather than the viewport's, and they are equal only because the shell happens to fill it.
