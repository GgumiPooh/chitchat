...
The tab bar is the one caller that dials it, to `1.08`. Its bloom sits on the tab's own anchor rather than the glyph stack inside (§ 7.3.), and that anchor is a quarter of the pill's width — at the 1.3 default it swells some 13px past the pill's `2xs` padding and over the tabs beside it. Note also that `scale: 1` makes every bloomed element a stacking context, so siblings paint in DOM order: a swelling control overlaps its later siblings from underneath, which only stays invisible while the swell is small.
...
Where it applies: the tab bar's anchor (§ 7.3.), every `icon-button` (§ 7.1.), and the composer's send disc (§ 6.6.). On the tab it is the anchor and not the glyph stack inside so the active tab's fill springs with its contents, instead of holding still while they bounce out of it. A circle 44px across sits entirely under the finger, so its fill change is invisible at the moment it matters and the scale is the only feedback the user can actually see. Rectangular buttons and chips (§ 7.1.) keep the fill change alone — they are wide enough that their edges stay in view.
...
| `:active` | The press bloom on the tab's anchor, dialled to `--press-scale: 1.08` (§ 4.7.2.)                            |
