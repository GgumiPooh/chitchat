...
| `--duration-tab-travel`  | 450ms                            | The tab fill crossing the bar, and the icon/label crossfade that lands with it (§ 7.3.) |
...
**Why the travel has a duration of its own, and why it is the longest committed state change in the app.** It ran on `--duration-state` (200ms) with everything else, and at that length it was the one state change users reported not seeing. The reason is distance, not speed: a toggle's knob crosses its own width and the bars collapse by their own height, while this crosses up to three quarters of the bar — the same duration therefore buys it several times the velocity, and the eye reads it as already-arrived rather than as having moved. 450ms is what makes the travel legible without reading as lag — 320ms was tried first and still read as a jump rather than a journey. It is now **more than double** `--duration-route-enter` (220ms), so the fill is unambiguously the last thing on screen to settle: the arriving screen is what the user is looking at, and the fill confirming behind it is the correct order for the two. That gap is the reason the length is affordable at all — the fill is confirmation of a switch the screen has already made, so it is free to take its time in a way nothing on the critical path could. Raising `--duration-state` instead is the wrong lever twice over — it would slow every toggle and hover fill in the app, and the bars' collapse is paired to the shell's own 200ms height ease (§ 3.4.), which is not a coincidence to break.
