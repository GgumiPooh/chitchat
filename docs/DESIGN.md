...
| Chat screen         | `fixed`, `top: var(--keyboard-pan)`, `height: var(--viewport-height, 100dvh)` — the only element the keyboard moves                                                                                                                                                                        |
| Chat screen motion  | `height` eases over 200ms `ease-out`; `top` never eases                                                                                                                                                                                                                                    |
| Chat composer       | `absolute` at `--bottom-inset` inside that box, so the keyboard carries it without a `fixed` offset of its own to correct (§ 6.6.)                                                                                                                                                         |
...
- The wrapper is **keyed**, or React reuses the node and the animation never restarts after the first navigation. The key counts **animated arrivals**, not pathnames: keyed on the path, 이모티콘 팩 A → 팩 B remounts the whole screen — every `useState`, scroll offset and in-flight query thrown away — for an animation that does not even run there.
  - **That key now also decides how much of `<Activity>` survives, and it is worth knowing which way round.** `REQUIREMENTS.md § 1.1.` keeps a departed route mounted at `display: none` instead of unmounting it, and this wrapper is an ancestor of every one of them — so a tab switch bumps the key, tears the whole subtree down, and the hidden routes go with it. Within a tab the direction is `none`, the key holds, and the previous screen is genuinely preserved. Neither half is arranged: it falls out of "animate between tabs, not within one", and it happens to leave 채팅 unable to hold a stale composer draft or an open picker across a tab switch. Do not change the key to the pathname to "fix" preservation — the bullet above is why, and it would take 이모티콘 팩 A → 팩 B down with it.
...
None of this applies to a CSS animation, because there are no snapshots. The bars are never captured, never composited apart, and never painted over: they are `fixed` outside the animating subtree and simply do not move. `body` keeps its own flat fill — `canvas` since § 3.3. — and `glass` keeps its blur (§ 3.5.); the blur is viable **because** nothing is captured, and reintroducing a capture takes it away again.
...
| Shadow        | None (§ 4.5.)                                                                                                                                 |
| Text wrapping | `whitespace-pre-wrap`, `break-words`, `overflow-wrap: anywhere`. `word-break: keep-all` is inherited from § 4.2.3. and nothing here resets it |
...
| Lines        | Sender name (`chat-time` in `chat-meta`) over the summary (`chat-name` at `leading-snug` in `meta-soft`). **Both `truncate`, always one line each** — and `REQUIREMENTS.md § 8.3.` measures those two class lists by name, so moving either moves the estimate                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| Summary copy | The text itself; `사진` / `동영상` / `파일` / `음성` for attachments, **counted past one** (`사진 2장`, `동영상 3개`) since a lone attachment is what the tile already shows; `이모티콘` for an emoticon, `삭제된 메시지예요` for a parent that has been deleted                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| Thumbnail    | The bubble's **first** attachment, or the emoticon itself — 32×32 either way, so `REQUIREMENTS.md § 8.3.` prices one box. An attachment is `rounded-xs` `object-cover` with a 1px `hairline` inset ring; an emoticon is `object-contain` with neither, its art being transparent-background and non-square (`REQUIREMENTS.md § 13.2.`) and a ring around it therefore a box drawn around nothing. Box fixed, never derived from the image (§ 6.1.). A file, a recording and a deleted parent have none. An attachment tile takes the `thumb` variant; an emoticon has no variant to take — `REQUIREMENTS.md § 13.2.` stores one image slot per item — so the tile is the very asset the bubble draws, shares its cache, and animates where that asset does. That is accepted rather than worked around: it is the same object the jump is pointing the reader at, and a still frame would need a canvas read of a cross-origin asset (§ 5.3.) |
...
**Every screen has one now, because every screen streams** (`REQUIREMENTS.md § 1.1.`). 캘린더 is `CalendarFallback`, 채팅 `ChatFallback`, 설정 `SettingsFallback`, 로그인된 기기 `DeviceSettingsFallback`, 이모티콘 관리 `EmoticonSettingsFallback`, a pack's own screen `EmoticonPackFallback`, and 보관함's three shelves keep `LibraryFallback`. The shell has one of its own, in the `(main)` layout, and it is the real `TabBar` rather than a skeleton of one — four glyphs and four labels that depend on nothing, so drawing a placeholder for them would be a swap for identical pixels.
...
**Two things are deliberately not drawn, and both for § 7.10.'s reason — but only one of them was ever safe.** 캘린더's 다가오는 일정 card renders nothing when there is nothing upcoming (§ 7.9.), and a pack screen's title is the name being fetched — a placeholder for either is a box that vanishes on roughly the occasions it was drawn for, and the eye settling on a layout and then having it taken away is worse than not drawing it. The count on 로그인된 기기 is the one number here that cannot be derived, so it is the one fallback a row may still arrive into. **Not drawing a section only settles what the fallback shows; it does not settle where the real one lands.** While 다가오는 일정 sat above the grid, this omission was also the reason the month stepped down the page as the screen streamed in — undrawn is only free at the foot of a screen, which is where § 7.9. now keeps it.
...
The calendar screen opens with a D-day header, then the month grid, then the selected day's agenda, and 다가오는 일정 last.

**Nothing above the grid may change height.** The grid is the screen's control surface — it is tapped and swiped repeatedly, and § 11.3. already pads every month to six rows so its own height cannot move under the thumb. That argument does not stop at the grid's top edge: 다가오는 일정 varies between nothing and three rows, and while it sat between the band and the grid the month moved day to day as events passed, again on every add or delete, and once more on arrival — the § 7.8. fallback draws no card, so the real screen inserted one above the grid and pushed it down. It is last now, where nothing follows it and its height costs nobody anything. The agenda varies too and always has; it is _below_ the grid, and it is the answer to the tap, which is why nothing may come between the two.
...
| Upcoming card  | **At the foot of the screen**, never above the grid. `canvas` fill, 1px `hairline`, `rounded-md`, padding `md`, under a `title-sm` `meta` `다가오는 일정` heading. Up to three entries: `title-sm` `ink` title, `caption` `meta` relative date. Hidden entirely when empty — never renders an empty-state here |
...
**Both bars are `absolute` over the track rather than rows beside it**, and that is what makes the chrome toggleable at all: a tap on the photo puts it away, and laid out as rows they would resize the track every time they left, so the photo would jump and re-snap under the very tap that hid it. Each bar is `pointer-events-none` with `auto` on its own controls — they span the full width over a photo whose taps toggle them and whose hold belongs to the OS (`REQUIREMENTS.md § 8.11.`), so an inert strip that still swallowed pointers would kill both gestures across the top and bottom of every slide. The top bar carries a `scrim/70`-to-transparent gradient so a name stays legible over a bright photo; the bottom controls carry their own `scrim/55` discs and `shadow-floating` instead, since a second gradient would frame the photo from below.
...
**A tap anywhere on the slide toggles the chrome, and nothing on the slide closes the viewer** — which is where it parts company with every other overlay (§ 3.5.1.). It dismisses on `Escape` and on 닫기 alone, both written out rather than inherited, since the viewer composes no `Dialog`. The scrim used to close it and the photo used to toggle, split by testing the tap against the **painted rectangle**: `object-contain` letterboxes the asset inside its `<img>`, so a portrait photo's side gutters look like scrim and are `<img>` to the DOM. The test was right and the behaviour was still wrong — on a portrait photo the gutters are most of what there is to tap, and `REQUIREMENTS.md § 8.1.`'s track is a stream the reader swipes through rather than a single photo they opened, so a tap a few pixels wide of the image threw them out of it. A stream is left deliberately; the two ways out are both aimed at. **The chrome starts visible**: the controls have to be findable without discovering the gesture first. The toggle must stay a `click` — a `pointerdown` would fire under the hold `REQUIREMENTS.md § 8.11.` deliberately leaves to iOS, and the chrome would vanish as the 사진에 저장 menu opened over it. A `<video>` is excluded whole, controls and gaps between them alike, and so is every control on a slide.
