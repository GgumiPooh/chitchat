...
| Right controls | Emoticon toggle (20px / 44×44), always present. The send button appears **beside** it once there is something to send — it does not replace it, because an emoticon can be staged alongside a typed line (`REQUIREMENTS.md § 13.6.`) |
...
| Emoticon picker chrome  | Panel height, pack tab-bar geometry, and grid density still unspecified. Assets are now user-authored (`REQUIREMENTS.md § 13.`), so the grid must hold arbitrary aspect ratios — a fixed square cell with the still `object-contain` inside it, rather than a ratio taken from the art |
