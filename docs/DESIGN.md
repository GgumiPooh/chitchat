...
| Chat screen         | `fixed`, `top: var(--viewport-top, 0px)`, `height: var(--viewport-height, 100dvh)` — the only element the keyboard moves                                                                                                                                                                   |
...
| Shell overlays      | A full-screen overlay is portalled into the shell, and `ShellOverlay` gives it a `fixed` box at `--viewport-top` of `--viewport-height` — the column it portals into is in flow (§ 3.3.). That box carries `z-40`, because `fixed` seals its children into a stacking context of their own |
