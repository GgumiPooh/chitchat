...
| Emoticon picker chrome  | Pack tab-bar geometry and grid density still unspecified. The panel is settled at half the shell's height (`--viewport-height`, § 3.4., never `vh`), tall enough that a pack reads as a grid rather than a strip. Assets are user-authored (`REQUIREMENTS.md § 13.`), so the grid must hold arbitrary aspect ratios — a fixed square cell with the still `object-contain` inside it, rather than a ratio taken from the art |
