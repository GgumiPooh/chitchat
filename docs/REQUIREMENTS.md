...
- [x] **Haptics on touch** (`DESIGN.md § 7.15.`). iOS has no Vibration API and iOS 26.5 removed the scripted-trigger workaround, so `HapticTap` is an invisible native `input[switch]` laid under the finger — an element, never a function call. It rides a `haptic` prop on `Button` / `IconButton` / `Chip` / `Switch` / `SettingsRow` / `Link`, and is composed by hand for the raw buttons (composer send, day cell, emoticon grid, list rows, `ActionSheet`). It fires on a committed change of state or a selection among peers, never on dismissal or back navigation, and **cannot** fire on a drag threshold such as swipe-to-reply (§ 8.10.) — no scripted trigger exists on 26.5+
...
Offscreen message nodes **must not stay in the DOM** — after years of history, thousands of infinite-scrolled nodes destroy scroll performance in iOS Safari. `@tanstack/react-virtual` is in place: `anchorTo: "end"` for jump-free prepends, `getItemKey` on the row key to anchor them, `measureElement` for variable heights, `scrollMargin` for the loading header above the list, and `scrollToIndex({ align: "center" })` for search jumps.

- **`flex-direction: column-reverse` is not available here, and the reason is structural.** It anchors a list from the bottom only because the browser lays the items out in normal flow; every row here is `position: absolute`, so it is out of flow and `flex-direction` cannot reach it. Buying the behaviour back means abandoning absolute positioning for spacers and inverting every `item.start` the virtualizer hands over. The `transform: scaleY(-1)` variant is no escape either — a transform flips painting, not input, so the scroll gesture ends up reversed. (`@tanstack/virtual-core` sets neither `overflow-anchor` nor flow spacers of its own; it is headless and hands over numbers only)
- **The prepend anchor is the row key, and it must be stable.** `getItemKey` returns `ChatRow.key`; the virtualizer captures the visible row before the data changes, finds the same key afterwards, and offsets the scroll by however far it moved. An index-based key cannot do this — every existing row takes a new index the moment a page lands in front of it
- **The room re-anchors the prepend itself, and never on `rows[0]`.** Left to the virtualizer, the anchor is whichever row sits at the scroll offset — and at the top of the list that resolves to index 0, which is a **date divider**. A page of older messages from the same day leaves that divider at index 0 with the same key and the same offset, so the correction computes as "nothing moved" and the entire page lands as drift; the scroller is then still at the top, asks for the next page, and the room walks backwards through history a page at a time. `insertOlder` records the first **non-date** row and an effect restores it to the offset it was recorded at. The target is absolute, never a delta, so it cannot double-apply on top of a correction the virtualizer did make
- **`anchorTo: "end"` holds the end; it does not go there.** At mount every row is still `ESTIMATED_ROW_HEIGHT`, so a single `scrollToEnd()` lands on an end the first measurement then moves. The room re-parks on the scroller's own maximum each render until a real gesture (`wheel` / `touchstart` / `keydown`) hands the scroll over — never gated on the at-bottom flag, which the edge sync has already read as false from the pre-park position
- **`estimateSize` is per row and must stay close, because a wrong estimate _moves the list_.** Every row first measured above the fold corrects the scroll by its own estimate error, and WebKit drops that correction mid-gesture — so one flat guess makes reading back through history drift by the accumulated error and snap back when the finger stops. `estimateRowHeight` resolves an attachment (`toMediaBoxHeight`) and an emoticon (`toEmoticonBox`) **exactly**, by sharing the arithmetic the bubble is drawn at rather than restating it; only text is approximated, from a glyph-width line count. Measured against the running app it lands within 0.05px on a text bubble and exactly on an emoticon, where the old flat `80` was off by −38 and +64
- **A page of older history is fetched during the scroll but inserted only once the scroller is still.** Inserting above the viewport needs a scroll correction of the inserted height, and WebKit hands the scroll offset to the compositor for the length of a gesture — a correction landing mid-flick is deferred to a scroll that has already moved on, which reads as a blank frame, then the room jumping into the past, then snapping back. `loadOlder` therefore holds its page and `useSettledCommit` inserts it after `SETTLE_DELAY` of no `scroll` and no finger down. The § 8.3. loading header stays up across the whole wait, and a window replacement (§ 8.6.1.) discards the held page rather than splicing live-edge history into a jumped window
- **Nothing the virtualizer measures knows about the trailing spacer, so nothing it scrolls to may be trusted as "the bottom".** `ListFooter` is `--chat-bottom-gap` of the scroll range and it is not a measured row, so `scrollToIndex`-based ends — which is what `followOnAppend` is — land exactly that far short and park the newest message behind the composer. Following the live edge is the scroller's own `scrollHeight`, keyed on the **last row's key** changing so a page of older history does not trip it
- **A § 8.9. link card is resolved a page ahead of the viewport, never on the bubble that needs it.** `useLinkPreviewPrefetch` asks the moment a message is in the loaded window, so `LinkPreviewCard` reads an answered cache and the bubble is its final height the first time it is measured. A card arriving after that grows the row **while the reader is looking at it**, and no scroll compensation can hide that — compensation moves the viewport with the content, and here the content the reader is on is what moved. Reserving a placeholder box instead is worse: most links describe themselves with nothing (§ 8.9.), so the box would usually collapse again
- **The default re-measure compensation is wrong for this list**, and `shouldAdjustScrollPositionOnItemSizeChange` replaces it. The library declines to compensate a row that is re-measured while the scroll direction is `backward` — but reading back through history is exactly that, and a § 8.9. link card resolving above the fold then shoves everything below it down by the card's own height. A row **entirely** above the fold is compensated whichever way the finger was moving; a row still spanning it grew below the anchor and must not be
- **Paging must never be driven from a render-scoped effect.** Both loads commit messages, which renders, which would ask again — one page per render rather than one per landed page, and the room pages itself to the start of history with the main thread pinned. Flags refresh on every render (`readScrollEdges`); paging runs on `scroll` and on the load finishing (`requestAdjacentPages`)
- Migrated off `react-virtuoso`, whose `firstItemIndex` model cost a long tail of traps: a `followOutput` callback could not disable its size-increase autoscroll (armed by the prop being `!== false`, which a function never is); `heightEstimates` and `defaultItemHeight` each armed an unguarded four-deep `requestAnimationFrame` recursion that made `GET /chat` a **500**; and a page landing before the mounted rows were measured was remapped against sizes it never took, leaving the whole page height as drift. None of these have an equivalent here
- **Its `ResizeObserver` measures without deferring a frame**, which `react-virtuoso` did not — a background tab gets no `requestAnimationFrame`, so the old list rendered nothing at all until the tab was foregrounded
...
- [ ] **R2 bucket configuration is a deploy step, not code** (§ 15.): the bucket must be private with public access fully disabled, and its CORS policy must allow `PUT` **and `GET`** from the app origin — without `PUT` every upload fails preflight, and without `GET` the gallery's share route (§ 10.) cannot read the bytes it hands to the share sheet. `AllowedHeaders` must cover **`Cache-Control`** as well as `Content-Type`: § 13.3. signs a `Cache-Control` into the emoticon PUT and the browser has to send it, and unlike `Content-Type` it is not a CORS-safelisted request header, so a policy that names headers one by one fails the preflight for every emoticon upload
...
- **The object is stored with its own `Cache-Control`** (`EMOTICON_OBJECT_CACHE_CONTROL`, a year, `immutable`), so the bytes cache as long as the redirect to them does instead of being revalidated on every load. It is **signed into the presigned PUT**, which makes it the one upload header besides `Content-Type` that the browser must reproduce byte for byte or R2 refuses the upload
...
- [ ] **R2 bucket setup**: private with public access disabled, plus a CORS policy allowing `PUT` and `GET` from the app origin — without `PUT` every § 9. upload fails preflight, without `GET` the § 10. share route cannot buffer an original, and no code change can fix either
