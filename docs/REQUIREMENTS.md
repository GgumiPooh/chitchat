...
- **Sending is not a trigger of its own.** It clears the staging and empties the field without producing an edit, so the signal lapses on its own; an explicit stop on send would wrongly hide an indicator that is still true when the panel stayed open
...
- **The indicator therefore trails the last keystroke by up to `TYPING_IDLE_AFTER + TYPING_TIMEOUT`.** That is deliberate: shortening it means either a stop event (which no frozen or killed sender ever sends) or a `TYPING_TIMEOUT` so tight that one dropped request blinks the indicator off mid-sentence
...
- **The indicator is the last thing in the conversation, not an overlay floating over it.** It sits in the scroller's own flow between the virtualized box and the trailing spacer, so it scrolls with the messages and reads as the bubble that is about to arrive — which is the whole point of it appearing where it does
- **It is still not a virtualized row.** `ListFooter` already establishes the position: content the scroller lays out itself, which the virtualizer never measures. A tail item that mounts and unmounts every few seconds would re-measure the list and drag the reader's scroll with it (§ 8.3.)
- **Mounted and unmounted, never faded in place.** It occupies real height at the tail, so a copy parked at `opacity-0` would hold a gap open under the newest message for as long as nobody was typing
- **A reader already at the bottom is followed to it**, in a layout effect. Content added below the fold does not move the scroller, so without this the indicator appears off-screen under the composer. Followed **only** from the bottom — someone reading history is not dragged to the present because the other person started typing
