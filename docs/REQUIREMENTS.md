...
- [x] The body is **read in chunks and stopped at `MAX_LINK_PREVIEW_BYTES`**, not `response.text()` — the `Content-Length` of a hostile response is a claim, and a stream that never ends would be buffered until the process ran out of memory. Only `text/html` is read at all
...
- [x] **YouTube goes through its own oEmbed endpoint**, not the watch page: that page renders its metadata from JS for an agent it does not recognise, so the generic path returns a title and no thumbnail. The channel name lands in the site line — the play badge on the tile already says which site it is
