...
- Picked attachments stage in a tray with a per-item remove control and an edit control on **every** tile — a photo crops and filters, a video trims. The trimmer is § 12.1.'s, with **no `maxDurationMs`**: an attachment has no length cap, so both handles move and trimming is an edit rather than a requirement
...
- Decoding is serial, so the tray opens on a **placeholder tile** rather than staying empty until the last file lands
