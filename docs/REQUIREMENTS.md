...
- [x] **The panel opens and closes over 200ms**, a `0fr`→`1fr` grid track rather than a mount. The history follows it, because `--chat-bottom-gap` is measured from the composer's box every frame the track grows — one animation drives both, with no second scroll animation to keep in step. Only the track knows the panel's height, so nothing has to restate it. The panel stays mounted after its first open (a collapse needs something left to animate) and is `inert` while closed, since a zero-height clip still holds tab stops
