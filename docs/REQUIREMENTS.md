...
- **The length cap is enforced by trimming, never by refusal.** A longer pick opens `VideoTrimmer` rather than being rejected, so the cap never costs a re-pick. A clip already inside the window is staged as it is, so the common case sees no extra screen
