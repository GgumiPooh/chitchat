...
| **Shared UI**                | `PreloadImage` and `PreloadVideo` are one shell copied twice; fold them into one                    | § 12.1.        |
...
- **Check `package.json` before adding a dependency.** R2, push, virtualization (`@tanstack/react-virtual`), text measurement (`@chenglou/pretext`), query, validation, image cropping (`react-advanced-cropper` — it replaced `react-easy-crop`, whose crop box cannot be resized), client storage (`synced-storage`), drag-and-drop (`@dnd-kit`) and browser-side media conversion (`mediabunny`, § 12.1.) are installed. **There is no remaining unmet dependency**; a new one needs the user's approval first
...
- **A file never reaches the gallery, the viewer, or a background.** `isInGallery()` adds `filename IS NULL`; `registerMedia` refuses a file under the `avatar` and `background` scopes and refuses `addToGallery`; the § 8.10. quote drops its thumbnail id and shows `파일` from `toMediaKind`; § 8.11.'s 공유 is withheld, because `useMediaShare` buffers originals for iOS's photo library and words every step of it as 사진
...
Dropping photos, videos and files anywhere over the chat room stages them in the composer's tray, exactly as a pick does.
...
- **A window-level `preventDefault` swallows drops that miss.** Unguarded, the browser navigates to the file, which in a standalone PWA replaces the app with a bare asset view and no way back
...
- Images **and videos** sent in Chat **appear here automatically** — `media` is the single source, nothing is copied. A row is in the gallery when it is not hidden **and** either a message that is still visible carries it, or it was uploaded straight here (`media.gallery_added_at`, § 6.). **A § 9.1. file attachment is excluded outright** (`filename IS NULL`): it has no `_thumb` object, so a tile of one is a broken image and the viewer opens on nothing
...
- **Saving the selection takes one of two routes, chosen by platform — and the platform that branches is iOS, not touch.** On iOS it is `navigator.share({ files })`, because the OS share sheet is the **only** way a web page reaches the iOS photo library: a `Content-Disposition: attachment` download lands in Files, where nobody who tapped 저장 over a photo will look. Everywhere else it is one top-level navigation per file, staggered by `GALLERY_DOWNLOAD_STAGGER`, because navigations started in one tick collapse into one. **Android is on the download side deliberately**: its share sheet has no OS-provided "save to gallery" action at all, only whichever apps are installed, while a download lands in `Download/` and MediaStore indexes it straight into the gallery — so there the sheet is the unreliable route and the download is the good one. The branch is `isIos()` **and** `navigator.canShare`, not either alone. It cannot be a pointer check: an iPad with a trackpad reports a fine pointer and still cannot get a download into Photos, and an Android phone reports a coarse one and does not need the sheet. It cannot be feature detection either — nothing exposes "will this download be findable", so this is the one sanctioned user-agent branch (`AGENTS.md § 4.2.`). (§ 8.11.'s 공유 shares the machinery but not this branch: it names the share sheet outright, so it uses one wherever there is one)
...
- **A pick is staged before it is uploaded.** The picker used to hand its files straight to the upload, which left no way to correct a bad frame or drop one item of forty. It now stages into `useMediaSelection` — the same hook the composer's tray uses (§ 8.1.) — so both screens edit an attachment the same way, and the `대화에도 보내기` / `갤러리에만 추가` choice sits under the tray rather than in front of it
...
- **A `<video>` with no decoded frame paints black, not nothing**, so every one in the app goes through `PreloadVideo` — `PreloadImage`'s contract for the element that cannot use it. Three rules that wrapper carries, each of them a bug it shipped without:
  ...
  - **Metadata counts as arrival, not only `loadeddata`.** A caller asking for `preload="metadata"` is asking the engine to stop at `HAVE_METADATA`, and where that is honoured literally `loadeddata` never fires — leaving the placeholder over a player that is working
...
- **Touching the history does not close the panel either.** It scrolls the history, and that is all it does. A `pointerdown` dismissal was tried and rejected: reading back through the conversation with the panel up is the ordinary way of picking what to reply to, and closing there both takes away the panel the user was about to use and runs the whole collapse — spacer, clearance and re-pin — underneath the finger that was only trying to scroll. The panel is closed by the toggle, or by reaching for the field
