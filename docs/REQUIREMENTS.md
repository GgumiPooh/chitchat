...
- [x] **One `AbortSignal` bounds the whole resolution, not each hop.** Armed inside the loop, four slow redirects cost four timeouts plus four DNS lookups, the platform's own function limit ends the request first, and nothing is written to the cache — so the next scroll onto the bubble pays it again
- [x] **Every quantifier before the `=` in the attribute regex is bounded.** `[\w:-]*\s*=` backtracks quadratically across a `<meta>` tag whose word characters never reach one, and the input is 512KB of whatever the linked host chose to send: a message linking to a host the sender controls would otherwise block the event loop for minutes. `MAX_LINK_PREVIEW_BYTES` is not a bound on that, it is the multiplier
...
- [x] **A YouTube URL's card is completed from its video id.** `findYouTubeVideoId` reads `/watch?v=`, `youtu.be/`, `/shorts/`, `/embed/` and `/live/`, and whatever the page gave up, the card is `video` and its thumbnail falls back to `hqdefault.jpg` — the one variant every video has, where `maxresdefault` is generated for some and 404s for the rest
