...
- [ ] **R2 bucket configuration is a deploy step, not code** (§ 15.): the bucket must be private with public access fully disabled, and its CORS policy must allow `PUT` **and `GET`** from the app origin — without `PUT` every upload fails preflight, and without `GET` the gallery's share route (§ 10.) cannot read the bytes it hands to the share sheet
