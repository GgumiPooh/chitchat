...
- `features/author-emoticon` reaches `features/upload-media` through **`@x/author-emoticon`**, never its barrel — two slices on the same layer, so the cross-import needs the FSD gate (`fsd/forbidden-imports`). `entities/emoticon/@x/message` is the same arrangement one layer down, and `features/upload-media/@x/send-message` and `@x/update-profile` are the third and fourth
...
- **Handing stored objects to the OS lives in `shared/share`**, not in `widgets/gallery-grid` where it was written. The gallery's 저장 (§ 10.) and a message's 공유 (§ 8.11.) are one route with two labels, and the two callers are a widget and a widget's sibling. The segment is `share` rather than `media` — `entities/media` already claims that name, and a segment matching a slice name is `fsd/ambiguous-slice-names`. Its dialog lives there too rather than in `shared/ui`: the hook toasts, so `shared/ui` importing the dialog would close a cycle between the two segments
- **`useLongPress` and `GESTURE_SLOP` live in `shared/lib`** for the same reason — chat's bubbles and the gallery's tiles both arm the gesture (§ 8.11., § 10.)
- **`MediaViewer` and `MediaCell` live in `shared/ui`.** `widgets/chat-room` and `widgets/gallery-grid` both render the fullscreen viewer, and a widget cannot import a sibling widget — so the shared layer is the only place the two can meet. `Avatar` is there for the same reason once it opens that viewer (§ 12.), and it could not move to `entities/user` regardless: that barrel exports a `server-only` api segment, so a client value-import from it is the ban `entities/message` already carries
...
Read `src/shared/lib/index.ts` for the inventory (nullish types, `safely*`, `mapPooled`, `assert`, `cn`, duration constants + Korean date formatters, DOM helpers, `useHydrated` / `useIsCoarsePointer` / `useIsVirtualKeyboardOpen`). Reach for it before writing a utility.

- **`safely*` and `mapPooled` sit under `lib/run/`**, not at the top level. `fsd/shared-lib-grouping` errors past 15 modules in `shared/lib`, so a new utility there either joins an existing group or makes one — both of these are "how a callback is run", which is the grouping.
...
- **The launch screen is markup-driven on iOS.** Chromium renders one from the manifest, but WebKit only honours `apple-touch-startup-image` links, and only when the image matches the launch surface exactly — so `shared/config/pwa.ts` holds the device table (`device-width`/`device-height` do **not** swap with orientation on iOS, so one row yields the portrait and landscape pair) and both `pnpm icons` and the root layout's `metadata.appleWebApp.startupImage` derive from it. The images are the `canvas` field with the wordmark centred, palette-quantised, under `public/icons/splash`. A device with no matching row is fine: iOS falls back to the manifest `background_color`, which is that same `canvas`
- **The launch screen only paints in standalone**, so the root layout MUST also declare `metadata.appleWebApp.capable` — and, because Next emits only the unprefixed `mobile-web-app-capable` from it, `metadata.other` carries the legacy `apple-mobile-web-app-capable` that iOS below 15.4 still reads. iOS caches this markup when the icon is added, so changing it means deleting and re-adding the home-screen icon before it takes effect
...
- [x] **Haptics on touch** (`DESIGN.md § 7.15.`). iOS has no Vibration API and iOS 26.5 removed the scripted-trigger workaround, so `HapticTap` is an invisible native `input[switch]` laid under the finger — an element, never a function call. It rides a `haptic` prop on `Button` / `IconButton` / `Chip` / `Switch` / `SettingsRow` / `Link`, and is composed by hand for the raw buttons (composer send, day cell, emoticon grid, list rows, staged-attachment removal, `ActionSheet`). It fires on a committed change of state or a selection among peers, never on dismissing a surface or going back, and **cannot** fire on a drag threshold such as swipe-to-reply (§ 8.10.) — no scripted trigger exists on 26.5+. A release that travelled `GESTURE_SLOP` is a drag, not a tap: the overlay refuses it, so neither the control nor the Taptic engine hears it (`DESIGN.md § 7.15.2.`). On a target that **tiles** a scroller (the month grid, the emoticon grid, a `SettingsRow`, the full-width name of a 이모티콘 관리 row) it needs `keepsScroll`, which moves the tap to a `<label>`: the switch keeps a drag of its own and would otherwise leave the surface unscrollable. That mode is opt-in on purpose — inside an `<a>` the anchor eats the label's activation and the tick vanishes (`DESIGN.md § 7.15.1.`)
- Anything that needs a **secure context** is unavailable when the dev server is reached at `http://<lan-ip>:3000`: service workers and push (§ 16.1.) simply do not exist there, and `crypto.randomUUID` is absent, which is why browser code mints ids through `randomId()` (`shared/lib/random.ts`) instead. Test those over the `pnpm dev:tunnel` https address
...
Landed: notch-corner bubbles (mine right, theirs left), avatar + nickname on the first message of a group, one timestamp per same-minute group on its last message, date divider pills (`오늘`, `어제`, `2026년 8월 3일 월요일`), the auto-growing composer with `+` and emoticon toggle (the send button appears beside the toggle once there is something to send, rather than replacing it), long-press / right-click → `ActionSheet` (copy / delete), link previews and tappable links in a text bubble (§ 8.9.), and the scroll-to-bottom button appearing ~200px from the newest message with an incoming count (`새 메시지 3`).
...
- **A media bubble arms the same hold every other bubble does** (§ 8.11.). It used to arm none, so that iOS's own callout could offer 사진에 저장 — but the callout opens _on top of_ the action sheet rather than instead of it, and two menus over one photo is not a choice the user can read. The app's sheet wins the hold and carries 공유, which reaches the same share sheet deliberately; the OS keeps the § 7.10. viewer, where nothing of the app's competes for it
...
- [x] Media messages render **without a bubble**; tapping opens a fullscreen viewer
  ...
  - [x] The composer's `+` opens a `BottomSheet` with the single row `사진/영상` → album. A `카메라` row was dropped: it needs a second input, since `capture` and `multiple` cannot be combined, and iOS already offers the camera from inside the picker that row opens
  ...
  - [x] A bubble's attachments upload through § 13.4.'s bounded pool. The **byte budget**, not the concurrency count, is what governs here: a bubble is up to `MAX_MEDIA_PER_MESSAGE` slots of `MAX_VIDEO_SIZE`, so videos still go out effectively one at a time while nine photos go out `UPLOAD_CONCURRENCY` wide. `mediaIds` is assembled **by index**, so the picked order survives however the uploads interleaved, and each landed id is committed as it arrives. A failure **stops the queue**: nothing further is launched, the tasks already in flight are allowed to settle, and only then is the error rethrown — so the retry is left exactly the remainder, and a bubble whose first attachment fails does not push the other eight before saying so. A slot the retry skips is weighed as zero, or its bytes would be reserved against the budget for an upload that never runs
...
**The jump itself landed with § 8.10.1.** — replying needed the same machinery first. What is left here is the search half: the result list, and navigating between results.

- [x] Tapping a result navigates to chat and **scrolls to that message**, loading context via `GET /api/messages?around={id}&limit=30`
- [x] **Reinitialize the infinite-scroll state around the jump point** so the user can keep scrolling both ways from there. It is the **window's** edges that are reset, not `newestKnownId` — § 8.10.1. gives the reason the two had to be split
...
- [x] The chat avatar reads `GET /api/media/{avatarMediaId}`, resolved from the participant set at render time like the name beside it (§ 12.)
...
A text message that contains a link renders a card above its text — thumbnail, title, and the site it came from (`DESIGN.md § 6.9.`). The link itself stays in the bubble as a tappable anchor.

- [x] **One card per bubble, from the first URL in the text.** A message pasted out of a share sheet routinely carries several, and a stack of cards buries the sentence the sender wrote
- [x] `GET /api/link-preview?url=` answers `{ preview }`, where `null` is the normal case rather than an error — most links describe themselves with nothing, and the bubble then renders exactly as it did before previews existed
- [x] **Signed-in only**, not because the metadata is secret: the handler makes an outbound request to a URL the caller chooses, and that is not something to leave open to the internet
- [x] **Cached in `link_previews`, keyed by the URL as it appears in the message**, minus the fragment — it never reaches the server, so two links differing only in it describe one page. Both sides drop it (`withoutFragment`), or the client would hold a query entry and spend a round trip per anchor against a single row. The row hangs off no message — two bubbles carrying the same link share one row, and deleting a message leaves the entry to expire on its own. `LINK_PREVIEW_TTL` for a resolved card, `LINK_PREVIEW_FAILURE_TTL` for a failure, which **is cached too**: without that row, a link to a host that is down costs a full timeout every time the bubble scrolls back into view (§ 8.3.)
- [x] **Only a scrape that _threw_ is `failed`; everything the scraper read and could make nothing of is `empty`** — a page with no tags, but equally a PDF, a 404, or a host the guard below refused. All of those are permanent answers, and filing them under the one-hour retry window would mean an outbound request per link per hour for the life of the conversation. `undefined` (threw) and `null` (read it, nothing there) are therefore kept apart from `fetchMetadata` all the way to `statusOf`
- [x] **Only a `failed` refetch falls back to the row it could not replace.** A page that has since dropped its tags is written as `empty` and answers with no card — served from the old row instead, a card would outlive the page that published it for as long as the link kept being scrolled past
- [x] **Every hop of the redirect chain is re-checked against `isPublicHttpUrl`** (§ 14.). `fetch` follows redirects on its own, which is exactly the wrong behaviour here: the URL comes out of a message body, so a link on a public host that `302`s to `http://169.254.169.254/` is an attacker-chosen request made from inside the deployment. The host is resolved with `node:dns` and **every** address it answers has to be public, so a hostname that simply points at a private range is refused. IPv6 is expanded to hextets rather than matched as text — `::ffff:7f00:1` and `0:0:0:0:0:0:0:1` are the same addresses as `::ffff:127.0.0.1` and `::1`, and a prefix test on the written form passes half of them
- [x] **One `AbortSignal` bounds the whole resolution, not each hop — and the oEmbed attempt is one of those hops.** Armed inside the loop, four slow redirects cost four timeouts plus four DNS lookups, the platform's own function limit ends the request first, and nothing is written to the cache — so the next scroll onto the bubble pays it again. The YouTube path is where this is easiest to lose: oEmbed and the watch-page scrape behind it are two calls, and giving each its own timer doubles the worst case for exactly the links that need the fallback. `fetchMetadata` arms the signal once and hands it to both
- [x] **Every quantifier before the `=` in the attribute regex is bounded.** `[\w:-]*\s*=` backtracks quadratically across a `<meta>` tag whose word characters never reach one, and the input is a megabyte of whatever the linked host chose to send — the ceiling the bullet below raised it to, which doubled this regex's worst case along with it: a message linking to a host the sender controls would otherwise block the event loop for minutes. `MAX_LINK_PREVIEW_BYTES` is not a bound on that, it is the multiplier
- [x] The body is **read in chunks and stopped at `</head>`**, with `MAX_LINK_PREVIEW_BYTES` as the ceiling for a document that never closes one — not `response.text()`, because the `Content-Length` of a hostile response is a claim and a stream that never ends would be buffered until the process ran out of memory. Only `text/html` is read at all. **The ceiling is a megabyte, and that is not slack**: YouTube's watch page puts its Open Graph tags around 670KB in, so at the original 512KB the tags were on the wire and the reader had already stopped — every YouTube link the oEmbed path could not answer got no card at all
- [x] **Open Graph first, Twitter's tags second, `<title>` last.** The document is scanned with regex rather than a parser: five tags out of a `<head>` do not justify a dependency and a full DOM, and nothing read is ever re-emitted as markup — React escapes every value at render
- [x] A page that declares a non-UTF-8 charset is **decoded again in the encoding it declares**. A Korean page served as EUC-KR read as UTF-8 puts mojibake in the card
- [x] **YouTube tries its own oEmbed endpoint first, then falls back to the watch page.** oEmbed answers the title, the channel and the thumbnail in a few hundred bytes, and the channel is what lands in the site line — the play badge on the tile already says which site it is. But it **answers 401 for a video whose uploader disabled embedding**, which is not a reason to give up: that video's watch page publishes the Open Graph tags like any other. The 401 is therefore swallowed rather than propagated (propagated, it would cache the link as a failure), and the scrape decides
- [x] **A YouTube URL's card is completed from its video id.** `findYouTubeVideoId` reads `/watch?v=`, `youtu.be/`, `/shorts/`, `/embed/` and `/live/`, and whatever the page gave up, the card is `video` and its thumbnail is `hqdefault.jpg` — the one variant every video has, where `maxresdefault` is generated for some and 404s for the rest. **The id-derived thumbnail replaces a ytimg one rather than merely filling an empty slot**: a watch page always publishes `og:image` as `maxresdefault.jpg`, so a fallback that only fires on a missing image never fires at all, and an SD video's card was left pointing at a 404 the tile then hid — the exact outcome the fallback exists to prevent. It also applies to a **title-less** page (a consent interstitial has no `og:title`) and to the **oEmbed** answer, since `statusOf` accepts a card carrying only a thumbnail and the id is in hand on both paths. `/embed/videoseries?list=…` is excluded: it is YouTube's canonical playlist embed and `videoseries` passes the id shape, which would force a `video` card pointed at a video that does not exist
- [x] **The thumbnail is rendered straight from the publisher's origin**, the one image in the app that is not an R2 object behind `/api/media` (§ 9.). `referrerPolicy="no-referrer"`, and a host that refuses to serve it hides the tile rather than leaving a broken frame in the bubble
- [x] The client holds the answer with `staleTime: Infinity` — it is already cached server-side for days, so a refetch on mount would be one request per scroll back onto the bubble that can only return what it returned before
...
- [x] **A pointer gets a hover button** beside the bubble on its outer side, the first of the pair § 8.11. adds 공유 to (`AGENTS.md § 4.2.`). It is positioned out of flow: in flow it would only exist while hovered, and its appearance would shove the bubble sideways under the cursor aiming at it. `hover:` already resolves under `@media (hover: hover)`, so a touch device never reveals it and never has to — it pulls the row instead, as the bullet below says
- [x] **Touch pulls the row sideways**, as in Instagram, rather than reaching reply only through the hold. It is the gesture the action sheet's first item was standing in for, and on an attachment — which held the hold open for the OS until § 8.11. took it back — it was for a while the only reply route there was. It engages only after 8px and only when the gesture leads horizontally, so the § 8.3. scroll keeps every ambiguous drag; once engaged it captures the pointer, which is why the hold disarms on movement and why the release suppresses the `click` it would otherwise end in — a pull that started on a photo must not also open the viewer
...
`공유` hands a message to the OS share sheet, and it is the same route § 10.'s 저장 already takes — `shared/share` is one implementation with two labels (§ 2.). Contract:

- [x] **Attachments go as files, a text message goes as its text.** An emoticon message offers no 공유 at all: it is a pack item both participants already have (§ 13.), not something the sender is handing over
- [x] **The action sheet is the touch half, a second hover button beside 답장 is the pointer half** (`AGENTS.md § 4.2.`), exactly as § 8.10. splits reply. 복사 now appears **only on a message that has text** — it was previously offered on an attachment bubble, where it copied an empty string
- [x] **The media bubble takes the hold back from the OS.** The old arrangement (§ 8.1.) left an attachment with no hold so that iOS's callout could offer 사진에 저장, and the sheet a hold does open — the row's, from a pull — then rendered underneath it. `LONG_PRESS_TARGET_CLASS` plus `draggable={false}` on every tile is what suppresses the callout, and 공유 inside the sheet is the deliberate route to the photo library that replaces it
- [x] **The viewer takes a 공유 control of its own**, beside 원본 저장. A fine pointer has no OS callout over the photo to hold, and 원본 저장 there lands in Files rather than in a sheet — so this is the pointer's route to the same place. It is hidden on a draft, which has no stored object for either control to reach, and absent on an avatar (§ 12.), which has no save control either
- [x] **The viewer dismisses on `Escape` and on a tap on the scrim.** It composes no `Dialog`, so what `Modal` inherits from Radix is written out — including the part Radix would have handled for free: the key is ignored while any overlay is open above it, or a confirmation dismissed with `Escape` would take the viewer under it along. A control on a slide is not scrim either, so the 원본 저장 an undecodable video falls back to still downloads instead of closing. The photo is not scrim — but its `<img>` box is not the photo either, since `object-contain` letterboxes inside it, so the tap is tested against the painted rectangle rather than the element (`DESIGN.md § 7.10.`)
- [x] **The § 7.10. viewer is where the OS hold survives.** It suppresses none of the native gestures, because a slide is the whole screen and the app has nothing there competing for the hold — so a photo opened full-screen still offers 사진에 저장, 복사, and everything else iOS puts in that menu. This is the one file that must never gain a long-press target class
- [x] **A hold that fires swallows the `click` its release ends in, and cancels the § 8.10. pull.** The finger is still down and still owns the gesture: without the first, holding a photo opens the § 7.10. viewer underneath the sheet the hold just opened; without the second, the same finger goes on pulling the row behind the sheet and stages a reply on release. `useLongPress` does the swallowing the way the pull already does, with an `onClickCapture` above the clickable, and only the pointer that armed the timer may clear the flag — a second finger landing while the sheet is up is not a new gesture
- [x] **Deleting an attachment bubble is confirmed wherever it was reached from**, the sheet included. One row is every photo in it (§ 6.), which is the one thing neither the sheet nor the viewer shows; a text bubble's 삭제 stays immediate
- [x] **공유 is not pointer-gated, unlike 저장.** § 10. branches away from the share sheet on a fine pointer because a desktop sheet offers mail and AirDrop where 저장 meant a file on disk — but that sheet is exactly what 공유 names, so it is used wherever `canShare` answers. Only its absence falls back, to the § 10. download for files and to the clipboard for text — as does a refusal of the text sheet, which has nothing buffered to hold for a second tap the way the files path does. Every fallback says which one it took: the 저장 wording and the 공유 wording are separate strings throughout
...
- [x] **The key is built server-side from the uploader's id** (`{scope}/{userId}/{uuid}`, `toScopePrefix`) and never read off the request. A signature the browser could aim is a signature that overwrites any object in the bucket. `POST /api/media` re-checks the prefix before it will claim an object, and matches the **scope** as well as the id — `MEDIA_UPLOAD_SCOPES` is `chat` and `avatar` (§ 12.) and deliberately not `emoticon`, or a § 13.3. object could be claimed as a `media` row and would then be a gallery row
...
- [x] **R2 bucket configuration is a deploy step, not code** (§ 15.): the bucket must be private with public access fully disabled, and its CORS policy must allow `PUT` **and `GET`** from the app origin — without `PUT` every upload fails preflight, and without `GET` the gallery's share route (§ 10.) cannot read the bytes it hands to the share sheet. `AllowedHeaders` needs **`Content-Type` only**, and no upload may add a second header without this line changing first: the live policy names headers one by one, so anything it does not list fails the preflight and the upload never reaches R2 at all. That is not a hypothetical — § 13.3. once sent a `Cache-Control` on the emoticon PUT and every emoticon upload failed until the header moved to the download side
...
- [x] Tapping opens the fullscreen viewer, which swipes across the **whole loaded gallery** rather than the month it was opened in. Holding a photo _inside_ the viewer reaches the OS's own menu — the grid suppresses that callout so the hold can select, and § 8.11. makes the viewer the one place it survives. It is the § 8.1. viewer, moved to `shared/ui` — the chat room and the gallery are sibling widgets and neither may import the other (§ 2.). Pinch zoom is still § 18. #6
...
- [x] **Multi-select is entered from the header control _or_ by holding a tile**, which picks that tile. It was header-only at first, on the argument that a tap must keep opening the viewer and that an accidental hold on a grid this dense is easy — but a tap still opens the viewer, since the hold swallows the `click` it ends in (§ 8.11.), and holding a photo to pick it is what every photo grid on the platform does. The hold is **touch-only** here: `contextmenu` is left alone, because the header control is already this gesture's pointer equivalent (`DESIGN.md § 3.2.`) and taking right-click would cost the browser's own image menu for nothing. It is withheld entirely while an upload is in flight, for the same reason the header control is disabled. `MAX_GALLERY_SELECTION` caps a selection, said in the UI rather than reported as a rejected request
- [x] **The hold arms a sweep: keep dragging and the whole range from the held tile to the one under the finger takes the hold's action**, in grid order, filling row by row like the platform's own picker. It is a **range**, not a trail — the tiles the finger's path happened to touch would leave a diagonal drag full of holes, and it is what lets pulling the finger back release what it has left behind. Every range is therefore applied to the selection as it stood when the hold fired, never to the last range. Holding a tile that is _already_ selected sweeps deselection instead, which is how a selection this large is corrected. The drag scrolls the grid itself while the finger rests within `EDGE_ZONE` of the scroller's edge, and re-tests what is under the finger on every frame the grid actually moved — a parked finger sends no move event, and there the tiles are what travels. It is **touch events, not pointer events**: only a non-passive `touchmove` can `preventDefault` the scroll the drag would otherwise be, and `touch-action` cannot stand in, since the finger is already down when the sweep arms and that property is read when the gesture begins. Four consequences of owning the gesture that far down:
  - **The listeners are attached inside the hold, never from an effect.** An effect runs a frame later, and a finger lifting inside that window is a `touchend` nothing is listening for — which would leave a document-wide `preventDefault` swallowing every later scroll, with a live anchor selecting behind it
  - **The sweeping finger is tracked by `Touch.identifier`**, adopted on the first move as the touch nearest where the hold fired — not `touches[0]`, which is a thumb that has been resting on the grid since before it. Only that finger's `touchend` ends the sweep, or, before the first move, a release within `GESTURE_SLOP` of the anchor, since any further drift would have cancelled the hold rather than fired it
  ...
  - **The cap truncates from the anchor outwards.** A sweep running _up_ the grid is ordered away from the finger, so keeping the first `MAX_GALLERY_SELECTION` would strip the held tile and everything under the finger and light up a distant block instead. The cap toast carries a fixed `id`, because a sweep held past the cap reaches it again on every tile
- [x] **Saving the selection takes one of two routes, chosen by pointer type.** (§ 8.11.'s 공유 shares the machinery and not this branch — it names the share sheet outright, so it uses one wherever there is one.) On a coarse pointer it is `navigator.share({ files })`: the OS share sheet is the **only** way a web page reaches the iOS photo library, since a `Content-Disposition: attachment` download lands in Files and nothing in the app can move it from there. Everywhere else it stays one top-level navigation per file, staggered by `GALLERY_DOWNLOAD_STAGGER`, because navigations started in one tick collapse into one. The branch is `useIsCoarsePointer` **and** `navigator.canShare`, not either alone — desktop Chrome answers `canShare` too, and its sheet offers mail and AirDrop where the tap meant a file on disk (AGENTS.md § 4.2.)
- [x] **The share route buffers the originals first, so it is bounded twice**: `MAX_GALLERY_SHARE_FILES` on the count, and `MAX_GALLERY_SHARE_BYTES` on the total, read from each response's `Content-Length` **before** the body is buffered — otherwise one 500MB video is in memory before the ceiling is noticed. Either bound, or any failed fetch, falls back to the download route rather than opening a sheet holding part of the selection, which would read as a successful save of all of it. This is what needs R2's CORS policy to allow GET as well as PUT (§ 15.)
- [x] **A share refused for a spent user activation asks for one more tap.** iOS spends the tap's transient activation on the first `await`, and buffering the selection is many — so `navigator.share` rejects with `NotAllowedError` for a selection that took long enough. The files are already in hand at that point, so the dialog offers `사진에 저장` and calls `share` straight out of that click, with nothing awaited before it
- [x] Direct upload from the gallery, with an option not to post it to the conversation (`대화에도 보내기` / `갤러리에만 추가`). Files upload through § 13.4.'s bounded pool, which is where the concurrency and byte-budget argument lives — the budget is what matters here, since this picker takes video and a `MAX_VIDEO_SIZE` file holds it alone. The pool answers in **pick order**, not completion order, and the post below depends on that (§ 18. #10.). Uploads always carry `gallery_added_at` — the user filed the photo here, so it must be in the grid whether or not the POST that follows succeeds, and it must survive that message later being deleted. A post longer than `MAX_MEDIA_PER_MESSAGE` splits into consecutive bubbles in order (§ 18. #10.)
...
- [x] Profile — change nickname, upload/change profile image (via R2). `PATCH /api/users/me`, reached from a 프로필 row in Settings; `features/update-profile` owns the sheet and reuses § 9.'s picker, editor and upload through `@x/update-profile`
  - [x] The nickname set here is exactly what appears as the sender name in chat and inside system sentences (§ 8.7.), initialized from the Google account name at first login and owned by the user thereafter. `MAX_NICKNAME_LENGTH` bounds it, and the body is trimmed **before** the length check — twenty spaces would otherwise pass and § 8.7. would fall back to the email local part for a name the user believes they set
  - [x] Saving needs no explicit broadcast — the UPDATE lands on `users`, so the § 6. trigger fires `user_changed` and every open screen refetches (§ 8.4.). The Settings screen itself is a Server Component, so it takes the new row from a `router.refresh()` instead
  - [x] **An avatar is a `media` row on the § 9. pipeline, under the `avatar/` key scope.** `MEDIA_UPLOAD_SCOPES` is what `POST /api/media/upload-url` will sign and what registration accepts a key under; `emoticon` stays out of it for § 13.3.'s reason. Such a row reaches neither the gallery (it has no `message_media` child and no `gallery_added_at`) nor the § 10. delete path, which is narrowed to what the grid actually shows
  - [x] **`ownsAllMedia` takes the scope, and each pipeline's objects stay inside it.** Ownership alone is not enough once there is more than one scope, and it fails in both directions: an `avatar/` object attached to a message acquires a `message_media` child under the row the next profile change deletes, which fails on that non-cascading key (§ 6.) and leaves its owner permanently unable to change their photo; a `chat/` object worn as an avatar puts that same delete in front of a photo a bubble still renders. `POST /api/messages` passes `chat`, `PATCH /api/users/me` passes `avatar`
  - [x] **`addToGallery` is refused outside the `chat` scope**, not ignored. It is the caller's own claim (§ 10.), and an avatar filed into the grid is deleted outright by § 10.'s removal — nothing carries it in a message — whereupon `users.avatar_media_id`'s `ON DELETE SET NULL` silently strips the wearer's photo
  - [x] **The crop is not optional and it is square.** The editor opens on the pick with `fixedAspectRatio={1}` and the ratio chips gone; backing out of it is backing out of the pick. A circle (`DESIGN.md § 7.7.`) crops anything else anyway, and the full-screen view would then show a framing the circle never did — squareness is also what lets that view reserve its box without a round trip for the stored dimensions
  - [x] **`canReadMedia` admits an object somebody is wearing.** Without it the other participant 404s on the avatar that names every bubble in front of them. The check is last, after the message join, so it costs nothing on the common path — and an avatar the owner has since replaced falls through it, which is what makes the swap real
  - [x] **The write reads the photo it replaces in the same statement**, through a `users` self-join whose alias Postgres resolves against the pre-update snapshot. A separate `SELECT` first cannot promise it: two devices saving different photos would both read the same row, and the one that lost would be reported by neither — orphaning its objects permanently, since `canReadMedia` admits only a **currently worn** avatar
  - [x] **The patch body must carry at least one key.** Both are optional, so `{}` parses, and the empty `.set()` behind it makes drizzle throw `No values to set` — a 500 for what is a malformed request. The UI cannot send one; a retry or any other client can
  - [x] **A replaced photo's row is deleted at once and its objects on the § 13.4. delay.** The other participant is holding the 302 the old id resolved to, cached for `MEDIA_CACHE_MAX_AGE` (§ 9.), so deleting the object with the row is a broken image rather than a stale one. `discardAvatarMedia` is narrowed to the caller's own `avatar/` prefix, not merely to their own rows: a crafted `PATCH` can aim `avatar_media_id` at any object its sender owns, and a chat photo reached that way would be deleted out from under the bubble carrying it — or fail outright on `message_media`'s non-cascading key (§ 6.)
...
- [x] **Tapping an avatar opens the photo full screen**, in the § 7.10. viewer — in chat, and on this screen. It is a single square cell with no save control: a profile photo is the person, not an attachment they shared. `Avatar` owns both halves behind `mediaId` and `canEnlarge`, and lives in `shared/ui` because `widgets/chat-room` and the settings screen both render it (§ 2.)
  - [x] **`canEnlarge` is off by default and stays off in the calendar day sheet.** That avatar sits inside the row button that opens the event (§ 11.4.), where a nested `button` is invalid markup and swallows the tap the row exists for
  - [x] **The button takes over sizing when it wraps the circle**, which is what keeps `className` meaning one thing either way. Left on the circle, a caller's `size-*` would size the wrapper while the circle kept its own — and the day sheet passes exactly that, so the trap is one flag away from being sprung
...
- **The asset redirect is cached for days, not minutes, and that is the difference from § 9.** The URL carries `v` (§ 13.4.), so it addresses one immutable version of one item — an edit produces a different URL rather than different bytes behind the same one, which is exactly what a `media` URL cannot say about itself. So the presigned GET is signed for `EMOTICON_URL_EXPIRY` (7 days, SigV4's ceiling) and the 302 is cached for `EMOTICON_CACHE_MAX_AGE` (6 days), still under the signature as § 9. requires. The old 5-minute pair meant a **freshly uploaded emoticon fetched over the network on every single mount** — R2 answers no `Cache-Control` of its own, so the browser fell back to a heuristic lifetime of a tenth of the object's age, which is nearly zero for one just uploaded, and any one of those fetches failing left a permanently broken image. Older objects hid the bug behind their own heuristic freshness, which is why it only ever showed up right after an upload
- **The bytes get their `Cache-Control` from the presigned GET, not from the object** (`EMOTICON_ASSET_CACHE_CONTROL`, a year, `immutable`, signed in as `response-cache-control`), so they cache as long as the redirect to them does instead of being revalidated on every load. Storing it on the object instead is the version of this that **broke every emoticon upload**: `Cache-Control` is not a CORS-safelisted request header, so the PUT that carried it needed the bucket's `AllowedHeaders` to name it (§ 9.) and failed preflight where the policy listed only `Content-Type`. Signing it as a response override asks nothing of the browser, needs no deploy step, and fixes the objects already in the bucket rather than only the next ones. Note it is **not** signed on the PUT either way — `cache-control` is in SigV4's always-unsignable set, so it never appears in `X-Amz-SignedHeaders`
...
- [x] **Edit an item after it is uploaded**, from the same sheet that authored it: replace the image, add a sound, remove a sound. `PATCH /api/emoticons/items/{id}` takes only the slots that changed — an absent `audioKey` keeps the sound the item has, an explicit `null` removes it — and a replaced image must arrive with its new measurements, since § 8.3. reserves the bubble's box from them. The objects the edit detaches are deleted from R2 **immediately**, by the same § 9. cleanup a delete uses. A deferred delete used to guard the other participant's cached redirect, and it cannot survive § 13.3.'s multi-day cache: an in-process timer that has to outlive a six-day window is not a timer, and waiting that long to free the bytes is worse than what it buys. `deleteObjectsAfterCacheWindow` still exists and is still right for § 12.'s replaced avatars, whose redirect stays on § 9.'s five-minute window — the emoticon path is the one that outgrew it. What replaces it is on the read side — `PreloadImage` retries a failed load once against a cache-busted URL, so a participant replaying a redirect to the object the edit removed pays one failed request and then sees the new image, instead of a broken one until the cache expires. Deleting an item is unaffected either way, since the row that rendered it is gone too
...
- [x] **Adding a pile of images at once**, one item per image. The pack screen's `+` opens a sheet offering 하나씩 추가 — the authoring sheet of the bullets above, unchanged — and 여러 장 한 번에 추가, which opens the picker with `multiple`. Several files go straight to registration with neither editor nor sound (a single file picked there still opens the authoring sheet) — both are single-item decisions, and a sound can be added afterwards by editing the item. Files are uploaded through a **bounded pool** (`mapPooled`), `UPLOAD_CONCURRENCY` wide and under a `MAX_UPLOAD_INFLIGHT_BYTES` ceiling. Both halves of that limit are load-bearing and this is the one place the argument is written down, since § 9. and § 10. now share the pool: a bare concurrency count cannot tell twenty 8MB emoticons from nine 500MB videos, and firing the latter four abreast is the timeout the original one-at-a-time loop existed to avoid. The byte budget always admits **one** task even when it alone exceeds the budget, or an upload larger than the ceiling would wait forever. **Registration is not pooled** — `POST` runs strictly in pick order behind the uploads, because `emoticon_items.sort_order` is assigned from `max(sort_order) + 1` at insert and racing the POSTs would shuffle the pack against the order the user picked in. The grid still fills in as each lands, and a per-file failure never aborts the rest. The progress line counts what is **left** (`n개를 더 올리는 중이에요`), decremented per settled file, because it sits beside a grid that is already filling in and a fixed total would contradict the rows next to it
- [x] **The `+` is disabled for as long as a pile is landing**, and it is the only route into the bulk path — the picker closes before the upload starts and the `ActionSheet` closes on select, so no second batch can be started. Two batches would each run their own in-order registration chain and interleave two picks in one `sort_order` sequence, and the screen's `setAddingCount(files.length)` is an absolute write the overlap would corrupt. § 10.'s gallery deliberately does the opposite — it allows concurrent batches and carries relative counters instead, because its grid is ordered by capture date and an interleave there is invisible
- [x] **A failed bulk add names the files it dropped**, in a `Modal` (`추가하지 못한 이모티콘`) rather than the count-only toast it used to be. A pile fails per file and for different reasons — `지원하지 않는 형식이에요`, `8MB를 넘어요`, `파일을 읽지 못했어요`, `업로드하지 못했어요`, `등록하지 못했어요` — and a bare count leaves the user re-picking twenty images to find the three that did not land. The list scrolls at a fixed height, since a pick of forty can fail forty times
...
- [x] **The closing tap re-pins the history when the finger comes up.** Opening is a tap on the toggle, so the strip's per-frame re-pin (below) lands; closing runs the same animation _under a finger that is still down_, and WebKit gives the scroll offset to the compositor for the length of that gesture — every re-pin made while the strip shrinks is dropped, and the newest message is left parked where the open panel had pushed it. A one-shot `pointerup` repeats the pin after the gesture, which is why it was intermittent rather than constant: it only bites when the finger outlives the 200ms collapse. It is `pointerup` and never `pointercancel` — the browser cancels the pointer exactly when it takes the gesture over as a scroll, and pinning there would yank the user out of the history they just started dragging through. For the same reason the pin's at-bottom test is read off the scroller **before** the spacer moves rather than from Virtuoso's `atBottomStateChange` flag alone, which lands a frame or more behind the animation and would otherwise strand the rest of it
- [x] **The pin is repeated once the strip's transition ends**, whichever way the panel was closed. The bullet above covers a finger that outlives the collapse; a send is the other way the per-frame pin is lost, and it does not need a finger at all — the post-send `scrollToBottom` is `behavior: "smooth"`, and on WebKit that animation goes on steering the scroller toward the offset the _open_ panel implied for the whole 200ms, overwriting every pin made underneath it. Closing the panel after sending an emoticon therefore left the history parked exactly where the open panel had pushed it. `transitionend` on the strip is the one moment its height is final and nothing else is animating; it is filtered on `propertyName === "height"` and on the strip being the event's own target, since the panel's transitions bubble through it
...
- [x] **The tab strip scrolls the selected tab back into view.** The swipe moves the selection without the finger ever touching the strip, and the remembered tab (above) can reopen the panel on a pack already past its right edge — in both cases the tab the grid is showing sits outside the strip's bounds and nothing on screen says which pack is open. The strip is scrolled by hand rather than with `scrollIntoView`, which walks _every_ scrollable ancestor: the clipping strip the panel lives in is `overflow: hidden`, and mid-collapse that counts as one, so a partially clipped panel would be scrolled inside its own clip and never come back. A small margin past the revealed tab leaves a sliver of its neighbour visible, so the strip still reads as scrollable where it stops
...
- [x] **The toggle puts the keyboard away itself.** That gate means a press made while the field is focused only sets a flag the panel is not allowed to act on, and iOS lowers the keyboard for a blur alone — a tap on a `<button>` is not one — so the press read as doing nothing at all, and the panel would only appear later, when the user happened to dismiss the keyboard by hand. The composer therefore blurs the field before it reports the toggle. The toggle is also inverted against **what is on screen** rather than against the flag behind it: while the keyboard suppresses the panel the flag can already be `true`, and inverting that closes a panel the user is asking to open
...
- [x] `POST /api/messages` verifies that every `mediaId` is a registered object **owned by the sender and uploaded under the `chat` scope** before attaching it. `message_media.media_id` is a foreign key, so an unregistered id would surface as a 500 rather than a 400; nothing else stopped one user hanging the other's objects off their own bubble; and the scope half is what keeps an avatar out of a message (§ 12.)
...
- [x] **R2 bucket setup**: private with public access disabled, plus a CORS policy allowing `PUT` and `GET` from the app origin — without `PUT` every § 9. upload fails preflight, without `GET` the § 10. share route cannot buffer an original, and no code change can fix either. `AllowedHeaders` is `Content-Type` only, so no upload may send a second header without this policy changing first (§ 9.)
...
- [x] A detected deploy reloads immediately **unless** the screen holds unsent work — composer text, staged attachments, a staged emoticon (§ 13.6.), a send in flight, or a bulk emoticon add still landing (§ 13.4.). Those register through `useUnsentWork`, and the reload retries every `APP_REFRESH_RETRY_DELAY` and on every backgrounding until they clear
- [x] **A bulk add counts even though nothing on screen would be lost.** The uploads are a plain promise chain, so routing away leaves them running to completion — but a document reload kills them mid-pile, and unlike a draft message there is no staged state left behind to resume from. Only a reload is held; the user's own refresh or app close is not something this can reach
...
5. **Chat tab (§ 8.) — in progress.** Static UI → message CRUD → infinite scroll → SSE ✅, read cursor ✅, replies and the jump they needed (§ 8.10., § 8.10.1.) ✅; the `1` marker and unread divider (§ 8.8.) and search (§ 8.6.) remain
...
10. **Settings tab (§ 12.) — in progress.** The profile editor (nickname and avatar) and the tap-to-enlarge avatar landed; the device list, 로그아웃's own row, and app info remain
...
| ~~5~~  | ~~Motion duration / easing token scale~~                                                   | —                    | **Decided: five durations and three curves** (DESIGN § 4.7.), with route transitions in § 4.7.1. and the press bloom in § 4.7.2. The 1.5s search flash and Vaul's own sheet timing are the two things still outside the scale                                                               |
