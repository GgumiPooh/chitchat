...
- **A media bubble arms no hold at all** (`DESIGN.md § 3.2.`). The OS share sheet is the only way a web page reaches the iOS photo library (§ 10.), and holding an attachment is how a user gets there — so the action sheet would be opening on top of the one gesture the bubble owes them. Reply is reached by the § 8.10. pull instead, and deleting one's own attachment moves into the § 7.10. viewer, confirmed, since it takes the whole bubble with it
...
- [x] **Touch pulls the row sideways**, as in Instagram, rather than reaching reply only through the hold. The pull is what an attachment has instead of a hold, which now belongs to the OS (`DESIGN.md § 3.2.`), and it is the gesture the action sheet's first item was standing in for. It engages only after 8px and only when the gesture leads horizontally, so the § 8.3. scroll keeps every ambiguous drag; once engaged it captures the pointer, which is why the hold disarms on movement and why the release suppresses the `click` it would otherwise end in — a pull that started on a photo must not also open the viewer
...
- [x] **Saving the selection takes one of two routes, chosen by pointer type.** On a coarse pointer it is `navigator.share({ files })`: the OS share sheet is the **only** way a web page reaches the iOS photo library, since a `Content-Disposition: attachment` download lands in Files and nothing in the app can move it from there. Everywhere else it stays one top-level navigation per file, staggered by `GALLERY_DOWNLOAD_STAGGER`, because navigations started in one tick collapse into one. The branch is `useIsCoarsePointer` **and** `navigator.canShare`, not either alone — desktop Chrome answers `canShare` too, and its sheet offers mail and AirDrop where the tap meant a file on disk (AGENTS.md § 4.2.)
