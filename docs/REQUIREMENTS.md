...
- **A pick is staged before it is uploaded**, and on desktop that pick can equally be a drop anywhere over the grid (§ 9.2.) — both land in the same tray, so the `대화에도 보내기` / `갤러리에만 추가` choice is reached the same way whichever route filled it. The picker used to hand its files straight to the upload, which left no way to correct a bad frame or drop one item of forty. It now stages into `useMediaSelection` — the same hook the composer's tray uses (§ 8.1.) — so both screens edit an attachment the same way, and the `대화에도 보내기` / `갤러리에만 추가` choice sits under the tray rather than in front of it
...
- **Touching the history closes the panel**, as reaching for the field does. A `pointerdown` on the scroller or a `wheel` over it dismisses; the toggle and the field still do too. **This reverses an earlier decision** — the panel used to survive a scroll, on the argument that reading back through the conversation with it up is the ordinary way of picking what to reply to, and that dismissing there runs the whole collapse under a finger that was only trying to scroll. The report that overturned it is the plain one: with the panel filling half the shell, touching the conversation reads as leaving the panel, and having it stay up reads as stuck. The collapse under the finger is the accepted cost
- **It is a gesture that dismisses, never the `scroll` event.** Opening the panel scrolls the history itself — the strip's growth re-pins it every frame — so a `scroll` listener closes the panel on the frame it opened. `pointerdown` covers touch and mouse alike; `wheel` covers the trackpad, which produces no pointer event at all. The listeners are mounted only while the panel is actually on screen
